    ...
    if(isset($_POST['submit'])){
        if(!isset($_FILES["image"])||$_POST['name']==''||$_POST['MSV']==''||$_POST['birthday']==''||$_POST['sex']==''||$_POST['phoneNumber']==''||$_POST['class']=='')
            echo "<script>alert('Phai nhap thong tin')</script>";  
                ...
                header('Location: dangkiphong.php?image=' . urlencode($generatedImageName) . '&name=' . urlencode($name). '&MSV=' . urlencode($MSV). '&birthday=' . urlencode($birthday). '&sex=' . urlencode($sex). '&phoneNumber=' . urlencode($phoneNumber). '&class=' . urlencode($class));
                exit;
        }
    }
    mysqli_close($con);
...
<body>
    <div class="flex">
        ...
        <form enctype="multipart/form-data" method="POST" class="flex flex-col gap-4">
            <h1 class="text-4xl font-semibold pb-5">Nhập thông tin sinh viên</h1>
